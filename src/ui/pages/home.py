...
try:
    from src.data.supabase_client import SupabaseClient, get_companies, get_top_revenue_companies
...
def render():
    ...
    # Header
    st.markdown('<h1 class="main-header">📊 미국 재무제표 분석 및 투자 인사이트 봇</h1>', 
                unsafe_allow_html=True)
    
    st.markdown('<p class="sub-header">AI 기반 미국 상장사 재무제표 분석 도구</p>', 
                unsafe_allow_html=True)
    ...
    with tab1:
        ...
        if SUPABASE_AVAILABLE and company_count > 0:
            try:
                ...
                if not top_df.empty:
                    # 데이터 포맷팅
                    display_df = top_df[['ticker', 'company_name', 'revenue', 'net_income', 'total_assets']].copy()
                    display_df.columns = ['티커', '기업명', '매출', '순이익', '총자산']
                    
                    # 숫자 포맷팅
                    display_df['매출'] = display_df['매출'].apply(lambda x: format_number(x))
                    display_df['순이익'] = display_df['순이익'].apply(lambda x: format_number(x))
                    display_df['총자산'] = display_df['총자산'].apply(lambda x: format_number(x))
    ...
    with tab2:
        ...
        search_query = st.text_input("티커 또는 기업명으로 검색", placeholder="예: AAPL, Apple, Microsoft")
            ...
            try:
                ...
                if not results.empty:
                    ...
                    for _, company in results.iterrows():
                        with st.expander(f"📊 {company['ticker']} - {company['company_name']}"):
                            # 기업 재무 정보 조회
                            financials = SupabaseClient.get_financial_summary(company['ticker'])
                            ...
                            if financials and financials.get('annual_reports'):
                                ...
                                with col1:
                                    st.metric("매출", format_number(latest.get('revenue')))
                                with col2:
                                    st.metric("순이익", format_number(latest.get('net_income')))
                                with col3:
                                    st.metric("총자산", format_number(latest.get('total_assets')))
                                
                                # 연도별 데이터 테이블
                                reports_df = pd.DataFrame(reports)
                                if not reports_df.empty:
                                    display_cols = ['fiscal_year', 'revenue', 'net_income', 'eps']
                                    available_cols = [c for c in display_cols if c in reports_df.columns]
                                    st.dataframe(reports_df[available_cols], hide_index=True)
                            else:
        ...
        **4. 💡 투자 인사이트**
        ...
        if SUPABASE_AVAILABLE and company_count > 0:
            ...
            with col2:
                st.markdown("**섹터별 분포**")
                # 섹터 정보가 있다면 표시
                if 'sector' in companies_df.columns and companies_df['sector'].notna().any():
                    sector_counts = companies_df['sector'].value_counts()
                    st.bar_chart(sector_counts)
                else:
                    st.info("섹터 정보가 아직 없습니다.")
        else:
            st.info("데이터를 수집하려면 '데이터 수집' 페이지로 이동하세요.")
